...
width = box_width * 2 + box_left_margin * 2 + interval
arrow_height = 4
arrow_width = 4
...
x = box_left_margin + box_width + interval
y = box * interval + top_margin
out = f'<rect x="{x}" y="{y}" rx="20" ry="20" width="{box_width}" height="{height}" style="fill:none;stroke:black;stroke-width:{line_width};" />\n'
output_file.write(out)

# Make label
text = "Internet"
out = f'<text x="{x + text_left_margin}" y="{y+text_y_offset}" class="my_style">{text}</text>\n'
output_file.write(out)

y1 = y + height * (2 / 3)
y2 = y + height * (2 / 3)
x1 = box_left_margin + box_width
x2 = box_left_margin + box_width + interval - arrow_height * 3
out = f'<line x1="{x1}" y1="{y1}" x2="{x2}" y2="{y2}" style="stroke:black;stroke-width:{line_width}" marker-end="url(#arrow)" />\n'
output_file.write(out)

y1 = y + height * (1 / 3)
y2 = y + height * (1 / 3)
x1 = box_left_margin + box_width + interval
x2 = box_left_margin + box_width + arrow_height * 3
out = f'<line x1="{x1}" y1="{y1}" x2="{x2}" y2="{y2}" style="stroke:black;stroke-width:{line_width}" marker-end="url(#arrow)" />\n'
output_file.write(out)
